...
#' based on a large-scale random sampling design. Returns an `sf` object with
...
#'
#' @title Generate Fine-Scale Sampling Design (random)
#'
#' @param data_random_locs_sf An `sf` object representing the large-scale
...
#' @author Murray
#' @export
sampling_design_fine_scale_random <- function(data_random_locs_sf, config_fine) {
  set.seed(config_fine$seed)
  data_random_locs_obs <- data_random_locs_sf |>
    dplyr::bind_cols(data_random_locs_sf |>
                sf::st_coordinates() |>
                as.data.frame() |>
