...
#'
#' Calculates reef-level hard coral cover (HCC) by projecting the spatially explicit 
#' synthetic hard coral cover field onto reef polygons. This function aggregates 
...
#' @param all_effects_hcc A matrix containing the synthetic hard coral cover field.
...
#'   \itemize{
#'     \item `data_reefs_sample_hcc` – HCC values at the sample-level.
#'     \item `data_reefs_hcc` – Reef-level HCC values in long format with `Year` and `Value`.
#'     \item `data_reefs_pts_hcc_sf` – Reef-level HCC values as an sf object.
#'   }
#'
#' @author Murray
#' @export
